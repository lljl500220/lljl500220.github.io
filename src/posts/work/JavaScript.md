---
icon: html
title: EPD研发中心前端开发规范JS篇
article: false
---
## 目录

EPD研发中心前端开发规范JS篇，本文引用了尤雨溪、Airbnb、standard、google等前端领域权威机构或个人对于代码风格和 质量管控的文章或条例，经过阅读贵州分支前端大部分项目，总结问题出现频率后得出的结论。
如对某部分有疑问，可以@luolj做解答。
...
- [1.2](#types--complex)  复杂类型: 复杂类型赋值是获取到他的引用的值。
  ...
  ```
  + 这部分尤其以object为特殊，我曾在酒店，景区，餐饮，人社以及石化处理过数次因为引用对象而导致的原始对象改变出现的bug。
...
- [2.1](#references--prefer-const) 所有的赋值都用 `const`，避免使用 `var`。eslint: [`prefer-const`](http://eslint.org/docs/rules/prefer-const.html), [`no-const-assign`](http://eslint.org/docs/rules/no-const-assign.html)
  ...
  const b = 2;
  ```
  不知道除酒店景区以及餐饮运营端外其它的项目是否还有使用var关键字的，如果有，请在自己的开发过程中修改写法。黔云通有相当多的var关键字定义的变量，在多个作用域中被进行重新定义，一不小心就会出现bug。
...
- [3.1](#objects--no-new) 使用字面值创建对象。eslint: [`no-new-object`](http://eslint.org/docs/rules/no-new-object.html)
  ...
  1. 创建一个新对象
  2. 将这个新对象链接到原型
  3. 执行对象的构造函数方法，对象的属性都会被添加到this的引用中
  4. 如果构造函数没有返回一个对象，则返回this，即对象本身；否则返回构造函数创建的对象  
  
  两者区别：  
  字面量不会调用new Object(),不执行其构造函数，简洁，语义化且性能更好。  
...
- [4.4](#arrays--from-iterable) 用 `...` 运算符而不是 [`Array.from`](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_Objects/Array/from) 来将一个可迭代的对象转换成数组。
  ...
  ```
  ::: info ES6迭代器
  有空看看[官方文档](https://zh.javascript.info/iterable)  
  这里说一下为什么使用...来拓展一个可迭代对象为真数组----"没啥好说的，简洁。"
  :::
...
- [5.1](#destructuring--object) 用对象的解构赋值来获取和使用对象某个或多个属性值。eslint: [`prefer-destructuring`](https://eslint.org/docs/rules/prefer-destructuring)
  ...
  > 为什么？ 解构使开发者不必为这些属性创建临时引用，并且避免重复引用对象。重复引用对象将造成代码重复、增加阅读次数、提高犯错概率。在一个块级作用域里，解构对象可以在同一个地方给解构字段赋值，而不需要读整个的代码块看它到底用了哪些字段。
...
- [7.3](#functions--in-blocks) 不要在非函数块（`if`、`while` 等）内声明函数。把这个函数分配给一个变量。浏览器会允许你这样做，但不同浏览器的解析方式不同，有很大概率造成混乱。eslint: [`no-loop-func`](http://eslint.org/docs/rules/no-loop-func.html)
...
- [10.2](#modules--no-wildcard) 不要用 `import` 通配符， 即 `*` 这种方式，除非在一些特定库中有明确说明需要如此。说的就是你Echarts！。
...
- [11.3](#generators--spacing) 如果你一定要用生成器，请确保它们的函数标志空格是得当的。eslint: [`generator-star-spacing`](http://eslint.org/docs/rules/generator-star-spacing)
...
- [13.1](#variables--const)  使用 `const` 或 `let` 声明变量。不这样做会导致全局变量。我们想要避免污染全局命名空间。eslint: [`no-undef`](http://eslint.org/docs/rules/no-undef) [`prefer-const`](http://eslint.org/docs/rules/prefer-const)
...
- [13.2](#variables--one-const) 为每个变量声明都用一个 `const` 或 `let `。eslint: [`one-var`](http://eslint.org/docs/rules/one-var.html)
  ...
  > 为什么？这种方式很容易去声明新的变量，你不用去考虑把 `;` 调换成 `,`。这种做法也可以是你在调试的时候单步每个声明语句，而不是一下跳过所有声明。
...
- [21.1](#21.1) **要分号！** eslint: [`semi`](http://eslint.org/docs/rules/semi.html)
  ...
  这里引用尤雨溪的一篇[文章](https://www.zhihu.com/question/20298345),曾经我也是一个坚定的不加分号主义者，直到我的代码出了bug(悲o(╥﹏╥)o),后来经过学习发现，只要我的代码中，没有任何以(,[,or这些关键字开头的代码，那么加与不加就没有任何的区别，原因也在上述内容中有过叙述。鉴于贵州分支中有部分项目中仍然存在着这种特殊符号开头的代码，所以此处我建议将分号加上为妙。
...
- [22.4](#coercion--comment-deviations) 请在注释中解释为什么要用移位运算和你在做什么。比如由于 `parseInt` 是你的性能瓶颈导致你一定要用移位运算。说明这个是因为 [性能原因](https://jsperf.com/coercion-vs-casting/3)。
...
- [23.4](#naming--leading-underscore) 不要用前置或后置下划线。eslint: [`no-underscore-dangle`](http://eslint.org/docs/rules/no-underscore-dangle.html)
  ...
  > 为什么？JavaScript 没有私有属性或私有方法的概念。尽管前置下划线在其它语言中通常的概念上意味着私有，事实上，这些属性在JavaScript上是完全公有的，因此这部分也是你的 API 的内容。这一概念可能会导致开发者误以为更改这个不会导致崩溃或者不需要测试。如果你想要什么东西变成私有，那就不要让它公共代码块出现。
...
- [23.5](#naming--self-this) 不要保存引用 `this`，用箭头函数或 [函数绑定——Function#bind](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function/bind)。
  ...
  ```
  这部分需要举个栗子，因为有些同学不是很能弄清楚bind这个玩意，实际是它是非常有用的，在数旅通中有相当多的代码引用了全局的this而不自知，所以一出bug就将变量提升到全局去，这在变成规范中是严格禁止的。
  ```javascript
