...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如上的代码,假设test都有可能不存在的话,查找test的顺序是什么样子的?</p><p>变量 test 的查找顺序：</p><p>当 b() 被调用时，它会按照以下顺序查找 test：</p><p>b() 自己的作用域：首先在 b() 内部查找 let test;。如果存在，则使用这个 test</p><p>a() 的作用域：如果 b() 内部没有 test，则会沿着作用域链向上查找 a() 的作用域，查找 a() 中的 let test;</p><p>全局作用域：如果 a() 中也没有 test，则会继续向上查找全局作用域</p><p>具体执行过程： c() 被调用，创建 c() 的作用域，其中包含 let test;</p><p>c() 调用 a()，a() 创建自己的作用域，其中包含 let test;</p><p>a() 返回 b()，c() 将 b() 赋值给 fn</p><p>c() 调用 fn()，即 b() D b() 开始执行，首先在自己的作用域内查找 test：</p><p>如果 b() 内部有 let test;，则使用这个 test</p><p>如果 b() 内部没有 test，则向上查找 a() 的作用域，查找 a() 中的 let test;</p><p>如果 a() 中也没有 test，则继续向上查找全局作用域</p><ol start="3"><li><p>computed中,是怎么收集副作用函数的 首先明确是运行时收集,大致流程如下: 1.</p></li><li><p>spa应用中,是怎么做到路由变化而页面不刷新的 使用了windows.history.pushState()或者window.history.replaceState()修改地址,并不会触发页面更新. 使用哈希方法,不会导致页面更新</p></li><li><p>问项目,最多后天,把所有项目整理一下,搞得自己项目都记不清</p></li></ol>`,18);function c(l,o){return s(),a("div",null,[e("more"),i])}const d=n(p,[["render",c],["__file","20250224智现未来.html.vue"]]);export{d as default};
